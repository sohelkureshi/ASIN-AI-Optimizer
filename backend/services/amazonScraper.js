...
class AmazonScraper {
  ...
  async scrapeFromMarketplace(asin, marketplace) {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'production') {
        // Production - Optimized for Render
        launchConfig = {
          args: [
            ...chromium.args,
            '--disable-gpu',
      ...
      page.setDefaultNavigationTimeout(45000); // 45 seconds instead of 60
      ...
      // Wait for product title with shorter timeout
      try {
        await page.waitForSelector('#productTitle, #title', { timeout: 10000 });
      } catch (e) {
        console.log('  Title selector not found, trying to extract anyway...');
      }
